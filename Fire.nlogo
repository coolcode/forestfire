...
breed [fires fire]    ;; bright red persons -- the leading edge of the fire
breed [embers ember]  ;; turtules gradually fading from red to near black
breed [persons person]  ;;
breed [exits exit]  ;;

persons-own [
  flockmates         ;; agentset of nearby persons
...
to setup
  ...
  create-persons population [  ;; create the initial persons
  ...
  ask persons with [pxcor <= min-pxcor + 5 ]
    [ setxy  xcor + 5  ycor]

  create-exits 1 [  ;; create the initial sheep
    setxy 90 -90
    set size 10
    set color red
    set shape "house"
...
;    [ stop ]
  ...
  ;; fire-ignite-tree
  wind-fire-ignite-tree

  ask persons [ flock ]
  ;; the following line is used to make the persons
  ;; animate more smoothly.
  ;; repeat 5 [ ask persons [bounce fd 0.2 ] display ]
  ;; for greater efficiency, at the expense of smooth
  ;; animation, substitute the following line instead:
  ask persons [bounce fd 1 ]
...
;; creates the fire tree
to ignite-tree  ;; patch procedure
  sprout-fires 1
    [ set color red ]
  set pcolor black
...
;; creates the fire person
to ignite-person  ;; patch procedure
  set my-neighbors (persons) in-radius 2
  ask my-neighbors
    [
      if safe = 0
        [ set color white
          set burned-persons burned-persons + 1
          die ] ]
end
...
to flock  ;; turtle procedure
  ...
  set neighbor-fires (fires) in-radius vision
  if any? neighbor-fires
    [ set color orange
      set label-color white
      set label "HELP!"
      set nearest-fire min-one-of neighbor-fires [distance myself]
      ;; backwards fire
      face nearest-fire
      set heading heading + 180
      escape-fires ]
      ;; turn-towards average-heading-towards-fires max-cohere-turn ]
      ;; turn-away ([heading] of nearest-fire) max-separate-turn ]

  set nearest-exit min-one-of exits [distance myself]

  if nearest-exit != 0 and distance nearest-exit < vision
    [ set color cyan
      set label "YE!"
      face nearest-exit ]

  if nearest-exit != 0 and distance nearest-exit < 3
    [ set color green
      set label ""
      set safe 1 ]
...
to-report average-heading-towards-fires
  let x-component mean [sin (towards myself)] of neighbor-fires
  let y-component mean [cos (towards myself)] of neighbor-fires
...
to escape-fires
  let direction mean [towards myself] of neighbor-fires
  set heading direction
  ; check near edge?
;  let edge-distance 5
;  if pxcor <= (min-pxcor + edge-distance)
;  [ set heading ((direction + 90)/ 2)
;    set label "<-"]
;
;  if pycor <= (min-pycor + edge-distance)
;  [ set heading ((direction + 180)/ 2)
;    set label "^"]
;
;  if pxcor >= (max-pxcor - edge-distance)
;  [ set heading ((direction + 270)/ 2)
;    set label "->"]
;
;  if pycor >= (max-pycor + edge-distance)
...
to bounce  ;; balls procedure
  if nearest-exit = 0 or distance nearest-exit >= vision
     [
        ;; bounce off left and right walls
;        if abs pxcor >= (max-pxcor - 3)
;        [ ifelse nearest-fire = 0
;          [ set heading (- heading) ]
;          [ face nearest-fire
;            set heading (heading + 90) ]
;        ]
;        ;; bounce off top and bottom walls
;        if abs pycor >= (max-pycor - 3)
;        [ set heading (180 - heading) ]

        let edge-distance 5
        if pxcor <= (min-pxcor + edge-distance)
        [ ; set heading ((heading + 90)/ 2)
...
to find-nearest-fire ;; turtle procedure
  set nearest-fire min-one-of fires [distance myself]
end

to escape-fire  ;; turtle procedure
  turn-away ([heading] of nearest-fire) max-separate-turn
  ;; back nearest-fire
end

to find-nearest-ember ;; turtle procedure
  set nearest-ember min-one-of embers [distance myself]
end

to escape-ember  ;; turtle procedure
  turn-away ([heading] of nearest-ember) max-separate-turn
end

to find-flockmates  ;; turtle procedure
  set flockmates other persons in-radius vision
end
...
to-report average-flockmate-heading  ;; turtle procedure
  ...
  let x-component sum [xcor] of flockmates
  let y-component sum [ycor] of flockmates
  ifelse x-component = 0 and y-component = 0
    [ report heading ]
    [ report atan x-component y-component ]
end
